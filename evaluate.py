...
async def a6(email, **kwargs):
    ...
    DATA_PATH = os.path.join(BASE_DIR, "data/docs/index.json")
    ...
    for dir, file, text in docs:
        ...
        for line in text.split("\n"):
            if line.startswith("# "):  # Extract first H1
                ...
                break
        if title:
            index_data[f"{dir}/{file}.md"] = title  # Map filename to title

    # Write index.json, overwriting any previous content
    try:
        async with aiofiles.open(DATA_PATH, "w") as f:
            await f.write(json.dumps(index_data, indent=4, sort_keys=True))
    except Exception as e:
        logging.error(f"❌ Error writing to {DATA_PATH}: {e}")
        return False

    # Validate written data
    try:
        result = await read(DATA_PATH)
        result = json.loads(result)
    except json.JSONDecodeError:
        logging.error("🔴 /data/docs/index.json was not valid JSON")
        return False

    if json.dumps(result, sort_keys=True) != json.dumps(index_data, sort_keys=True):
        return mismatch("/data/docs/index.json", index_data, result)
...
async def extract_credit_card_number(image_path):
    """Extracts credit card number from an image using OCR"""
    try:
        img = Image.open(image_path)
        extracted_text = pytesseract.image_to_string(img)

        # Extract only numbers from OCR output
        extracted_number = re.sub(r"\D", "", extracted_text)
...
async def a8(email, **kwargs):
    ...
    output_file = await extract_credit_card_number(image_path)

    if not output_file:
        return mismatch("/data/credit-card.txt", data["number"], "OCR extraction failed")

    # Read extracted number from file
    with open(output_file, "r") as f:
        result = f.read().strip()
    
    if not result:
        return mismatch("/data/credit-card.txt", data["number"], "File is empty")

    # Extract only digits for comparison
    extracted_number = re.sub(r"\D", "", result)
    expected_number = re.sub(r"\D", "", data["number"])

    if extracted_number != expected_number:
        return mismatch("/data/credit-card.txt", expected_number, extracted_number)
